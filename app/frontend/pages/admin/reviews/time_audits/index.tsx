...
const pendingColumns: ColumnDef<ReviewRow>[] = [
  {
    ...
    header: 'Project',
    cell: ({ row }) => (
      <Link href={`/admin/reviews/time_audits/${row.original.id}`} className="font-medium hover:underline">
...
const allColumns: ColumnDef<ReviewRow>[] = [
  {
    ...
    header: 'ID',
    cell: ({ row }) => (
      <Link href={`/admin/reviews/time_audits/${row.original.id}`} className="text-muted-foreground hover:underline">
        {row.original.id}
      </Link>
    ),
  },
...
export default function TimeAuditsIndex({
  pending_reviews,
  all_reviews,
  pagy,
}: {
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        <h2 className="text-lg font-semibold tracking-tight mb-3">All Time Audits</h2>
...
TimeAuditsIndex.layout = (page: ReactNode) => <AdminLayout>{page}</AdminLayout>
