...
module ShipChecks
  module ImagesShow3dModel
    DEFINITION = { key: :images_show_3d_model, label: "README shows 3D model or completed build", deps: [ :image_descriptions ], visibility: :internal }.freeze

    def self.call(ctx)
      descriptions = ctx.image_descriptions
      if descriptions.nil? || descriptions.empty?
        return ShipCheckService::CheckResult.new(
          key: "images_show_3d_model", label: DEFINITION[:label],
      ...
      response = chat.ask(<<~PROMPT)
        You are reviewing a hardware project submission. Based on these image descriptions from the project's README, does at least one image show a 3D model, render, or photo of the completed build in real life?
